...
export default function CargasRecientes({
  ...
  const cargar = useCallback(() => {
    setCargando(true);
    const params = new URLSearchParams({ creadoPor: nombreVisible, limite: "8" });
  ...
  async function eliminar(id: number, nombre: string) {
    if (!confirm(`¿Eliminar "${nombre}"? Útil si fue una carga de práctica/capacitación. No se puede deshacer.`))
      return;
    setEliminandoId(id);
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-azul-100 bg-white shadow-card">
      <div className="border-b border-azul-100 bg-azul-50/60 px-4 py-2.5">
        <p className="text-sm font-bold text-azul-900">Sus cargas recientes</p>
        <p className="text-xs text-slate-500">
          Si registró algo de prueba o de una capacitación, puede eliminarlo aquí.
        </p>
      </div>
          ...
          items.map((a) => (
            <div key={a.id} className="flex flex-wrap items-center gap-3 px-4 py-3">
              <div className="min-w-[180px] flex-1">
                ...
                <p className="text-xs text-slate-400">Cargada el {formatoFecha(a.createdAt)}</p>
              </div>
